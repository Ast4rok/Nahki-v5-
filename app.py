...
# ── FUNÇÃO CORRIGIDA PARA A GROQ VISION API ──
def build_messages_for_api(messages, system_prompt=""):
    """
    Constrói a lista de mensagens para a API.
    FORÇA a injeção de uma string no índice 0 (role: system) para evitar o 
    Erro 400 da Groq quando a primeira mensagem do usuário contém imagens.
    """
    api_msgs = []
    
    # Injetamos obrigatoriamente um system prompt como String pura no índice 0
    # Se não houver um prompt definido, usamos um valor padrão
    final_sp = system_prompt.strip() if system_prompt and system_prompt.strip() else "Você é um assistente útil e prestativo."
    api_msgs.append({"role": "system", "content": final_sp})
    
    # As mensagens do chat seguem a partir do índice 1, permitindo multimodal (listas)
    for msg in messages:
